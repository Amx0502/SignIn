    ...
    <el-dialog v-model="editorVisible" :title="editingId ? '编辑自动任务' : '新增自动任务'" width="620px" align-center append-to-body>
      <el-form label-position="top">
        <div class="form-grid">
          ...
          <el-form-item label="每日执行时间" class="wide">
            <div class="schedule-list">
              <div v-for="(_, index) in draft.schedule_times" :key="index" class="schedule-row">
                <el-time-picker v-model="draft.schedule_times[index]" value-format="HH:mm:ss" format="HH:mm:ss" placeholder="选择执行时间" />
              ...
              <el-button plain @click="draft.schedule_times.push('08:00:00')">添加时间</el-button>
            </div>
          </el-form-item>
          <el-form-item label="执行日期范围" class="wide">
            <div class="date-row">
              <el-date-picker v-model="draft.start_date" value-format="YYYY-MM-DD" type="date" placeholder="开始日期（可选）" />
              <span>至</span>
              <el-date-picker v-model="draft.end_date" value-format="YYYY-MM-DD" type="date" placeholder="结束日期（可选）" />
            </div>
          </el-form-item>
          <el-form-item class="wide"><el-checkbox v-model="draft.notify_wecom">发送企业微信通知</el-checkbox></el-form-item>
...
.panel-head { display:flex;align-items:center;justify-content:space-between;gap:14px }.panel-head strong,.panel-head small,.task-name strong,.task-name small { display:block }.panel-head strong{font-size:16px}.panel-head small,.task-name small,.muted{margin-top:4px;color:#64748b;font-size:11px}.task-name strong{color:#172033}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 16px}.form-grid .wide{grid-column:1/-1}.el-select,.el-input-number{width:100%}.coordinate-row,.date-row,.schedule-row{display:flex;align-items:center;gap:10px;width:100%}.coordinate-row .el-input,.date-row .el-date-editor,.schedule-row .el-date-editor{flex:1}.field-tip{display:block;margin-top:7px;color:#64748b}.schedule-list{display:grid;gap:10px;width:100%}
