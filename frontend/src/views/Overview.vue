    ...
    <el-row :gutter="18" class="analytics-grid">
      <el-col :xs="24" :lg="14">
        <el-card class="chart-card" shadow="hover">
          <template #header>
            <div class="card-header"><span class="card-title">任务健康度</span><el-tag type="success">{{ completionRate
            }}%</el-tag></div>
          </template>
          <div class="progress-list">
            <div class="progress-row">
              <div class="progress-meta"><span>启用任务占比</span><strong>{{ completionRate }}%</strong></div>
              <div class="progress-track">
                <div class="progress-fill" :style="{ width: `${completionRate}%` }"></div>
              </div>
            </div>
            <div class="progress-row">
              <div class="progress-meta"><span>账号任务覆盖</span><strong>{{ coverageRate }}%</strong></div>
              <div class="progress-track">
                <div class="progress-fill" :style="{ width: `${coverageRate}%` }"></div>
              </div>
            </div>
            <div class="progress-row">
              <div class="progress-meta"><span>调度可用性</span><strong>{{ state.auto_enabled ? 100 : 35 }}%</strong></div>
              <div class="progress-track">
                <div class="progress-fill" :style="{ width: `${state.auto_enabled ? 100 : 35}%` }"></div>
              </div>
            </div>
          </div>
        </el-card>
      ...
      <el-col :xs="24" :lg="10">
        <el-card class="chart-card" shadow="hover">
          <template #header>
            <div class="card-header"><span class="card-title">运行状态</span><el-tag
                :type="state.auto_enabled ? 'success' : 'danger'">{{ state.auto_enabled ? '自动调度已开启' : '自动调度未开启'
                }}</el-tag></div>
          </template>
...
.status-orbit {
  ...
  display: grid;
  place-items: center;
  background: radial-gradient(circle, #dbeafe 0 32%, transparent 33%), conic-gradient(from 110deg, #2563eb, #06b6d4, #93c5fd, #2563eb);
  border-radius: 22px;
  padding: 10px;
}
