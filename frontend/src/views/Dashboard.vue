...
const locationQuotas = computed(() => {
  ...
  return [{ key: 'legacy', label: '腾讯地址调用', used, limit, remaining: limit > 0 ? Math.max(0, limit - used) : '不限', percent: limit > 0 ? Math.min(100, Math.round(used * 100 / limit)) : 0, date: locationUsage.value.date }]
})
