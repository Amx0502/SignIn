<template>
  <div class="cube-subpage"><el-card><template #header><div class="page-head"><span>账号管理</span><el-button type="primary" @click="openQrLogin">微信扫码添加账号</el-button></div></template>
    <AccountCheckinPanel :accounts="accounts" :courses="courses" :items="items" :tasks="tasks" :selected-account-id="selectedAccountId" :selected-course-id="selectedCourseId" :selected-item-id="selectedItemId" :selected-course="selectedCourse" :selected-item="selectedItem" :courses-loading="coursesLoading" :items-loading="itemsLoading" :upload-photo-action="uploadPhoto" :manual-checkin-action="manualCheckin" :batch-delete-accounts-action="removeAccounts" @qr-login="openQrLogin" @select-account="selectAccount" @select-course="selectCourse" @select-item="value => selectedItemId = value" @sync-courses="syncCourses" @sync-items="syncItems" @rename-account="renameAccount" @delete-account="removeAccount" />
