...
const manualCoordinate=ref(''),manualLocationName=ref(''),manualNotify=ref(true),manualChecking=ref(false),manualAnswers=ref({}),profileSaving=ref(false)
...
const RunList=defineComponent({props:{runs:{type:Array,default:()=>[]},details:{type:Boolean,default:false}},setup(p){return()=>p.runs.length?h('div',{class:'run-list'},p.runs.map(v=>{const summary=v.request_summary||{};const location=summary.location_info;const content=[h('b',v.message||v.status),h('span',`${v.trigger==='manual'?'手动执行':'自动调度'} · ${format(v.started_at)}`)];if(p.details&&location)content.push(h('small',{class:'run-audit'},[`上游操作：${summary.operation||v.operation||'createBaomingByInput'}`,h('br'),`项目 ID：${summary.tongji_id||'—'}`,h('br'),`locationInfo.street：${location.street||'未单独提交'}`,h('br'),`locationInfo.name：${location.name||'未填写'}`,h('br'),`提交坐标：${location.lattitude}, ${location.longtitude}`,h('br'),`秒应记录 ID：${v.remote_submission_id||'未返回'}`]));return h('article',{class:`run ${v.status}`},[h('i'),h('div',content),h('em',v.status==='success'?'成功':'失败')])})):h('div',{class:'empty-inline'},p.details?'暂无签到审计日志':'暂无运行记录')}})
...
async function loadAccountForms(){accountForms.value=selectedAccountId.value?await api.listForms(selectedAccountId.value):[];if(!accountForms.value.some(item=>item.id===selectedFormId.value))selectedFormId.value=defaultFormId(accountForms.value);syncManualProfile();manualCoordinate.value='';manualLocationName.value='';manualAnswers.value=seedAnswers(manualFields.value,{...selectedAccount.value,...manualProfile},{})}
...
const isMissingLocationName=value=>!String(value||'').trim()||String(value).trim()==='地图选点'
const locationNameFromResult=result=>String(result?.name||result?.address||'').trim()
...
async function selectManualLocation(result){manualLocationName.value=locationNameFromResult(result);try{manualLocationName.value=locationNameFromResult(await reverseLocation(result))||manualLocationName.value}catch{}}
async function selectTaskLocation(result){form.location_name=locationNameFromResult(result);try{form.location_name=locationNameFromResult(await reverseLocation(result))||form.location_name}catch{}}
const radians=value=>Number(value)*Math.PI/180
function pointDistance(a,b){const radius=6371000;const dLat=radians(b.latitude-a.latitude),dLng=radians(b.longitude-a.longitude);const value=Math.sin(dLat/2)**2+Math.cos(radians(a.latitude))*Math.cos(radians(b.latitude))*Math.sin(dLng/2)**2;return 2*radius*Math.asin(Math.min(1,Math.sqrt(value)))}
async function snapToNearbyLandmark(point,displayName){const parts=String(displayName||'').split('-',2).map(value=>value.trim());if(parts.length<2||!parts[1])return{point,snapped:false};try{const response=await api.searchLocations(parts[1],'',8);const candidates=Array.isArray(response?.data)?response.data:[];const exact=candidates.find(item=>String(item.name||'').trim()===parts[1]&&pointDistance(point,item)<=2000);if(!exact)return{point,snapped:false};const target={latitude:Number(exact.latitude),longitude:Number(exact.longitude)};return{point:target,snapped:pointDistance(point,target)>2}}catch{return{point,snapped:false}}}
async function submitManualCheckin(){const item=selectedManualForm.value;if(!item)return;if(!answersValid(manualFields.value,manualAnswers.value)){ElMessage.warning('请完整填写项目必填项');return}let point={latitude:null,longitude:null};if(item.requirements?.location){try{point=parseCoordinates(manualCoordinate.value)}catch(e){ElMessage.warning(e.message);return}}manualChecking.value=true;try{if(item.requirements?.location&&isMissingLocationName(manualLocationName.value))manualLocationName.value=locationNameFromResult(await reverseLocation(point));if(item.requirements?.location){const resolved=await snapToNearbyLandmark(point,manualLocationName.value);point=resolved.point;if(resolved.snapped){manualCoordinate.value=`${point.latitude}, ${point.longitude}`;ElMessage.info('已使用附近同名地标的腾讯 POI 中心坐标')}}if(!await saveManualProfile(false))return;const result=await api.manualCheckin(item.id,{account_id:selectedAccountId.value,location_name:manualLocationName.value.trim(),answers:manualAnswers.value,...point,notify_wecom:manualNotify.value});ElMessage.success(result.message||'签到成功');if(result.notification?.reason==='send_failed')ElMessage.warning('签到成功，但企业微信通知发送失败')}catch(e){ElMessage.error(e.message)}finally{manualChecking.value=false}}
...
async function saveTask(){if(selectedTaskForm.value?.requirements?.identity?.fixed&&!form.answers.__identity){ElMessage.warning('请选择项目的班级和姓名');return}if(!form.account_id||!form.form_id||!form.name){ElMessage.warning('请选择账号和签到项目并填写任务名称');return}if(selectedTaskForm.value?.requirements?.unsupported?.length){ElMessage.warning(`暂不支持：${selectedTaskForm.value.requirements.unsupported.join('、')}`);return}if(!answersValid(taskFields.value,form.answers)){ElMessage.warning('请完整填写项目必填项');return}try{if(selectedTaskForm.value?.requirements?.location&&form.latitude!=null&&form.longitude!=null){const current={latitude:form.latitude,longitude:form.longitude};if(isMissingLocationName(form.location_name))form.location_name=locationNameFromResult(await reverseLocation(current));const resolved=await snapToNearbyLandmark(current,form.location_name);form.latitude=resolved.point.latitude;form.longitude=resolved.point.longitude;if(resolved.snapped)ElMessage.info('已使用附近同名地标的腾讯 POI 中心坐标')}const fn=editingId.value?api.updateTask(editingId.value,{...form}):api.createTask({...form});await fn;ElMessage.success('任务保存成功');resetForm();await load()}catch(e){ElMessage.error(e.message)}}
...
watch(selectedFormId,()=>{manualCoordinate.value='';manualLocationName.value='';manualAnswers.value=seedAnswers(manualFields.value,{...selectedAccount.value,...manualProfile},{})})
