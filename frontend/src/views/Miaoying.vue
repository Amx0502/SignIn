...
const RunList=defineComponent({props:{runs:{type:Array,default:()=>[]},details:{type:Boolean,default:false}},setup(p){return()=>p.runs.length?h('div',{class:'run-list'},p.runs.map(v=>{const summary=v.request_summary||{};const location=summary.location_info;const content=[h('b',v.message||v.status),h('span',`${v.trigger==='manual'?'手动执行':'自动调度'} · ${format(v.started_at)}`)];if(p.details&&location)content.push(h('small',{class:'run-audit'},[`上游操作：${summary.operation||v.operation||'createBaomingByInput'}`,h('br'),`项目 ID：${summary.tongji_id||'—'}`,h('br'),`locationInfo.name：${location.name||'未填写'}`,h('br'),`提交坐标：${location.lattitude}, ${location.longtitude}`,h('br'),`秒应记录 ID：${v.remote_submission_id||'未返回'}`]));return h('article',{class:`run ${v.status}`},[h('i'),h('div',content),h('em',v.status==='success'?'成功':'失败')])})):h('div',{class:'empty-inline'},p.details?'暂无签到审计日志':'暂无运行记录')}})
