...
function selectManualLocation(result){if(isMissingLocationName(manualLocationName.value))manualLocationName.value=locationNameFromResult(result)}
function selectTaskLocation(result){if(isMissingLocationName(form.location_name))form.location_name=locationNameFromResult(result)}
async function resolveLocationName(point){const result=await api.reverseLocation(point.latitude,point.longitude);const name=locationNameFromResult(result);if(!name)throw new Error('未识别到该坐标的位置名称，请手动填写');return name}
async function submitManualCheckin(){const item=selectedManualForm.value;if(!item)return;if(!answersValid(manualFields.value,manualAnswers.value)){ElMessage.warning('请完整填写项目必填项');return}let point={latitude:null,longitude:null};if(item.requirements?.location){try{point=parseCoordinates(manualCoordinate.value)}catch(e){ElMessage.warning(e.message);return}}manualChecking.value=true;try{if(item.requirements?.location&&isMissingLocationName(manualLocationName.value))manualLocationName.value=await resolveLocationName(point);if(!await saveManualProfile(false))return;const result=await api.manualCheckin(item.id,{account_id:selectedAccountId.value,location_name:manualLocationName.value.trim(),answers:manualAnswers.value,...point,notify_wecom:manualNotify.value});ElMessage.success(result.message||'签到成功');if(result.notification?.reason==='send_failed')ElMessage.warning('签到成功，但企业微信通知发送失败')}catch(e){ElMessage.error(e.message)}finally{manualChecking.value=false}}
...
async function saveTask(){if(selectedTaskForm.value?.requirements?.identity?.fixed&&!form.answers.__identity){ElMessage.warning('请选择项目的班级和姓名');return}if(!form.account_id||!form.form_id||!form.name){ElMessage.warning('请选择账号和签到项目并填写任务名称');return}if(selectedTaskForm.value?.requirements?.unsupported?.length){ElMessage.warning(`暂不支持：${selectedTaskForm.value.requirements.unsupported.join('、')}`);return}if(!answersValid(taskFields.value,form.answers)){ElMessage.warning('请完整填写项目必填项');return}try{if(selectedTaskForm.value?.requirements?.location&&form.latitude!=null&&form.longitude!=null&&isMissingLocationName(form.location_name))form.location_name=await resolveLocationName({latitude:form.latitude,longitude:form.longitude});const fn=editingId.value?api.updateTask(editingId.value,{...form}):api.createTask({...form});await fn;ElMessage.success('任务保存成功');resetForm();await load()}catch(e){ElMessage.error(e.message)}}
