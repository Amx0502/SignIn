      ...
      <div class="panel editor"><header><h2>{{ editingId ? '编辑任务' : '新建自动签到任务' }}</h2></header>
        <el-form label-width="88px"><el-form-item label="任务名称" required><el-input v-model="form.name" /></el-form-item>
          ...
          <el-form-item label="签到位置"><div class="location-editor">
            ...
            <LocationSearchPanel v-model="taskCoordinate" :location-api="api" />
          </div></el-form-item>
...
const taskCoordinate=computed({
  ...
  set:value=>{try{const point=parseCoordinates(value);form.latitude=point.latitude;form.longitude=point.longitude;if(!String(form.location_name||'').trim())form.location_name='地图选点'}catch{form.latitude=null;form.longitude=null;if(!String(value||'').trim()&&form.location_name==='地图选点')form.location_name=''}},
})
