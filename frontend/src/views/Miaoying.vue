    ...
    <section v-else-if="isAccounts" class="accounts-workspace">
      ...
      <div class="panel checkin-pane">
        ...
        <div v-else-if="selectedManualForm" class="manual-checkin">
          ...
          <div class="manual-actions"><div><el-checkbox v-model="manualNotify">发送企业微信通知</el-checkbox><small>自动识别只补充空缺字段，手动填写的位置内容优先</small></div><el-button type="primary" size="large" :loading="manualChecking" :disabled="!canManualCheckin" @click="submitManualCheckin">执行签到</el-button></div>
        </div>
      </div>
    </section>
